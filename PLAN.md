...
Status: planning only. This document describes the next implementation stages; it does not change the current shimmed language service.

## Goal

Use Binny as a safe reader of compiler-produced Nim semantic artifacts while keeping the LSP protocol and runtime in Nimdex/Sigils.
...
Binny's native-artifact workflow documents a Nim devel compiler with `--genBif` and `nifler`. The current development environment is Nim 2.2.12 and does not provide `nifler`; the plan must not make server startup or the current shim depend on those tools.

The first experiment must record the exact compiler, command line, Nim cache layout, and artifact names that produce usable semantic `.s.bif` files. Do not assume that `nim check --genBif:on` is sufficient. If the installed compiler cannot produce the required artifacts, the server must continue to run the shim and report semantic analysis as unavailable rather than failing initialization.
...
Completion criterion: a checked-in or reproducible fixture plus a written compatibility matrix. No LSP capability is promoted based only on synthetic BIF data.
...
Keep the existing language shim as the fallback when no usable artifact exists. Full document synchronization updates the authoritative overlay, but must not pretend that the compiler artifact has been rebuilt from unsaved text.
...
The first hover implementation should return a clear `null` or “analysis unavailable” result when the current open buffer differs from the indexed source and no overlay analysis exists. It must not return stale positions as if they described the current buffer.
...
- probes and records compiler capabilities;
...
2. The current environment without `nifler` still starts and serves the shim.
...
- Which supported Nim compiler/devel revision is the minimum for reproducible BIF generation?
